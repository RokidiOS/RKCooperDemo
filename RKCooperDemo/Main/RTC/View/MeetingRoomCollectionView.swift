...
var isf = false
class RKCollectionView: UICollectionView {

}

class MeetingRoomCollectionView: UIView {
    
    weak var collectionView: RKCollectionView!
    ...
    func setupView()  {
        let collectionView = RKCollectionView(frame: .zero, collectionViewLayout: UICollectionViewFlowLayout())
    ...
    func updateAudioCell(_ userId: String, bitrate: Int32, networkQualityState: RKINetworkQualityState)  {
        _ = collectionView.visibleCells.first { cell in
            guard let cell = cell as? MeetingRoomCollectionCell else { return false }
            if cell.info?.userId == userId {
                var nq = " NQ: --"
                if networkQualityState == .EXCELLENT {
                    nq = " NQ: 优"
                } else if networkQualityState == .GOOD {
                    nq = " NQ: 良"
                } else if networkQualityState == .BAD {
                    nq = " NQ: 差"
                }
                                
                cell.audioInfoLabel.text = "audio \(bitrate) Kpbs NQState: \(nq)"
                return true
            }
            return false
        }
    ...
    func updateCell(userId: String, width: Int32, height: Int32, fps: Int32, rid: String, bitrate: Int32, qualityLimitationReason: String?, packetsLost: Int32? = nil, networkQualityState: RKINetworkQualityState) -> String{
        ...
        if networkQualityState == .EXCELLENT {
            newInfoString.append(" NQ: 优")
        } else if networkQualityState == .GOOD {
            newInfoString.append(" NQ: 良")
        } else if networkQualityState == .BAD {
            newInfoString.append(" NQ: 差")
        } else {
            newInfoString.append(" NQ: --")
        }
...
extension MeetingRoomCollectionView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.userNameLabel.text = roomMember.userId == ContactManager.shared.selfUserId  ? "我" : roomMember.userId
//        kcontactList.first(where: { model in
//            model == roomMember.userId
//        }) ?? "我"
        ...
        let showDoodleFlag: Bool = channel.shareInfo?.shareType == .doodle || channel.shareInfo?.shareType == .imageDoodle
        ...
        if roomMember.state?.isEmpty == false, showFlag {
            ...
            participant.startVideo(renderType: .RENDER_FULL_SCREEN, videoSize:.SIZE_LARGE) { [weak cell]  canvas in
                if let canvasView = canvas?.videoView {
                    guard let sp = canvasView.superview, sp == cell?.videoView else {
