...
class MeetingManager: NSObject {
    ...
    var defaultSubscribeMediaType: SubscribeMediaType = .both
    
    var defaultStreamType: Bool = false
    ...
    func createMeeting(meetingName: String,
        ...
        channelParam.defaultStreamType = defaultStreamType
        channelParam.defaultSubscribeMediaType = defaultSubscribeMediaType
    ...
    private func joinMeeting(channelId: String, onSuccess: RKOnSuccess?, onfailed: RKOnFailed?) {
        ...
        RKChannelManager.shared.join(channelId: channelId, channelPassword: nil,onSuccess: { data in
            ...
            self.channel = channel
            // 接受邀请
            RKCooperationCore.shared.getCallManager().accept(channelId: channelId, defaultSubscribeMediaType: self.defaultSubscribeMediaType, defaultStreamType: self.defaultStreamType, onSuccess: onSuccess, onfailed: onfailed)
        }, onFailed: onfailed)
    }
...
extension MeetingManager {
    ...
    // MARK: - 发起会议
    public func startMeeting(userIds: [String], _ vc: UIViewController) {
